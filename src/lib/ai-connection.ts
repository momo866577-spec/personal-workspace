...
export async function callProvider(connection:Pick<AiConnection,"provider"|"model"|"baseUrl">,apiKey:string,prompt:string,maxTokens=800,system?:string){
 const base=connection.baseUrl.replace(/\/$/,"");let response:Response;
 if(connection.provider==="gemini")response=await fetch(`${base}/models/${encodeURIComponent(connection.model)}:generateContent`,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":apiKey},body:JSON.stringify({systemInstruction:system?{parts:[{text:system}]}:undefined,contents:[{role:"user",parts:[{text:prompt}]}],generationConfig:{maxOutputTokens:maxTokens}})});
 else if(connection.provider==="claude")response=await fetch(`${base}/messages`,{method:"POST",headers:{"Content-Type":"application/json","x-api-key":apiKey,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:connection.model,max_tokens:maxTokens,system,messages:[{role:"user",content:prompt}]})});
 else response=await fetch(`${base}/chat/completions`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${apiKey}`},body:JSON.stringify({model:connection.model,messages:[...(system?[{role:"system",content:system}]:[]),{role:"user",content:prompt}],max_tokens:maxTokens})});
...
const replySystem="你是一位直播高情商话术助手。使用者输入的是直播情境。不要解释，不要分析，不要前言，不要后记，直接输出主播可以在直播间讲出去的话。严禁出现‘作为AI’、‘作为直播助手’、‘以下提供’、‘我建议’。";
const promptFor=(mode:AiToolMode,input:string,style="高情商")=>mode==="reply"?`当前偏好风格：${style}。严格输出三种版本，不得添加其他内容：\n① 亲切版\n「一句可直接说的话」\n\n② 幽默版\n「一句可直接说的话」\n\n③ ${style}重点版\n「一句可直接说的话」\n\n直播情境：${input}`:mode==="review"?`分析以下直播记录：${input}\n请用清晰小标题给出做得好的地方、需要改善、可替换说法、提高互动率、提高停留率和最终复盘结论。不要自我介绍，不要写无意义前言。`:`直播情境：${input}\n直接给出最佳回复、第二种回复、高情商版、幽默版、带货版、引导成交版，并在每条后用一句短句说明有效原因。不要自我介绍或写前言。`;
...
export async function generateWithConnectedAi(mode:AiToolMode,input:string,style?:string){const connection=await db.aiConnections.get("active"),apiKey=await readSavedKey();if(!connection||!apiKey)throw new Error("尚未接入 AI，请先到设置 → AI 中心完成连接");const output=await callProvider(connection,apiKey,promptFor(mode,input,style),800,systemFor(mode));return {output:mode==="reply"?cleanReply(output):output,provider:connection.provider}}
