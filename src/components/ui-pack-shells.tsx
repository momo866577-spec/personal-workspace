...
export function JellyVisionShell({page,title,nav,go,children}:Props){
  ...
  return <div className="v3-shell jelly-product bunny-life">
    <aside className="jelly-dock"><button className="jelly-logo" onClick={()=>go("dashboard")} aria-label="返回总览"><Sparkles/><small>工作台</small></button>{nav.map(item=><motion.button whileTap={{scale:.86}} whileHover={{scale:1.08}} key={item.id} className={page===item.id?"active":""} aria-current={page===item.id?"page":undefined} onClick={()=>go(item.id)}><item.icon/><span>{item.label}</span></motion.button>)}</aside>
