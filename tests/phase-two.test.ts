...
test("CEFR lessons change by date and level while keeping the required item counts",()=>{const first=builtInLesson("2026-08-03","A1"),next=builtInLesson("2026-08-04","A1"),advanced=builtInLesson("2026-08-03","C1");assert.equal(first.words.length,5);assert.equal(first.sentences.length,3);assert.notDeepEqual(first.words,next.words);assert.notDeepEqual(first.words,advanced.words)});

test("imported question bank schema is validated",()=>{const valid={id:"test-bank",name:"测试题库",version:"1.0.0",source:"test",importedAt:new Date().toISOString(),lessons:[{id:"a1-1",level:"A1",words:["hello","world","today","learn","happy"],sentences:["Hello world.","I learn today.","I am happy."],shadowing:"Hello world.",speaking:"Introduce yourself.",listening:"Listen and repeat."}]};assert.equal(validateQuestionBank(valid).id,"test-bank");assert.throws(()=>validateQuestionBank({...valid,lessons:[]}))});
