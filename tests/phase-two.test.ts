...
test("workout recommendations search each recorded action on three platforms",()=>{const chest=workoutGuides({bodyPart:"胸部",exercise:"卧推"}),legs=workoutGuides({bodyPart:"腿部",exercise:"深蹲"});assert.deepEqual(chest.map(x=>x.platform),["Keep","Pamela Reif","周六野 Zoey"]);assert.match(chest[0].url,/keep\.com\/search\?keyword=/);assert.match(decodeURIComponent(chest[0].url),/胸部 卧推/);assert.match(decodeURIComponent(legs[1].url),/Pamela Reif Leg Workout/);assert.match(decodeURIComponent(legs[2].url),/周六野 腿部 深蹲/);for(const guide of [...chest,...legs])assert.match(guide.url,/^https:\/\//)});
