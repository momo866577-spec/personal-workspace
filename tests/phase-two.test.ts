...
test("v4 JSON exports and imports all old and new tables",async()=>{const exported=await exportAllData();for(const name of ["tasks","english","workouts","notes","streams","contacts","workoutCheckins","englishDailyPlans","aiToolRecords","englishQuestionBanks","periodRecords"])assert.ok(Array.isArray(exported[name]));const count=(exported.englishDailyPlans as unknown[]).length;await importAllData(exported);assert.equal(await db.englishDailyPlans.count(),count);db.close()});
