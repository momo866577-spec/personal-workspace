...
test("NGSL curriculum is fixed, refresh-safe, and advances after completion",async()=>{await Dexie.delete("personal-workspace");await db.open();const firstDate="2026-08-03",nextDate="2026-08-04";await ensureEnglishDailyPlan(firstDate);await ensureEnglishDailyPlan(firstDate);const first=await db.englishDailyPlans.get(firstDate);assert.equal(await db.englishDailyPlans.count(),1);assert.deepEqual(first?.items[0].prompts?.map(x=>x.text),["the","be","and","of","to"]);assert.equal(first?.source,`${NGSL_VERSION} · 固定顺序`);assert.equal(first?.items[0].prompts?.every(x=>Boolean(x.phonetic&&x.translation&&x.example&&x.exampleTranslation)),true);await db.tasks.where("due").equals(firstDate).modify({done:true});await ensureEnglishDailyPlan(nextDate);const next=await db.englishDailyPlans.get(nextDate);assert.equal(next?.items[0].prompts?.[0].text,"a");const tasks=await db.tasks.toArray(),plans=await db.englishDailyPlans.toArray(),valid=new Set(plans.map(x=>x.date)),stats=curriculumStats(tasks,valid,nextDate);assert.equal(stats.completedWords,5);assert.equal(stats.level,"A1");assert.equal(stats.chapter,2);assert.equal(stats.total,NGSL_TOTAL)});
...
test("workout recommendations use external platform links by training area",()=>{const chest=workoutGuides({bodyPart:"胸部",exercise:"卧推"}),legs=workoutGuides({bodyPart:"腿部",exercise:"深蹲"});assert.deepEqual(chest.map(x=>x.platform),["Keep","Pamela Reif","周六野 Zoey"]);assert.equal(chest[0].title,"胸肌入门");assert.equal(legs[1].title,"Leg Workout");for(const guide of [...chest,...legs])assert.match(guide.url,/^https:\/\//)});
